...
## Writeup
First, I extracted modulus from the .pub files:
```bash
...
q2 = n2//p
```
After p and q were obtained, all I had to do was find $d$ such that $de\equiv 1$(mod $\phi$) and calculate $c^d$ (mod $n$).
...
In the decrypted plain text, there is a flag:
```
